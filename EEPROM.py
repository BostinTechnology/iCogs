...
def WriteMapVersion():
    ...
    bus.write_byte_data(0x50, 0x00, 0x00)
    time.sleep(0.5)
    byte_h = bus.read_byte_data(0x50, 0x00)
    bus.write_byte_data(0x50, 0x01, 0x02)
    time.sleep(0.5)
    byte_l = bus.read_byte_data(0x50, 0x01)
...
def WriteDeviceBus():
    ...
    bus.write_byte_data(0x50, 0x10, 0x01)
    time.sleep(0.5)
    byte = bus.read_byte_data(0x50, 0x10)
...
def WriteSensorI2CAddress():
    ...
    bus.write_byte_data(0x50, 0x11, 0x5f)
    time.sleep(0.5)
    byte = bus.read_byte_data(0x50, 0x11)
...
def WriteBlanks():
    ...
    success = True
    for x in add_list:
        bus.write_byte_data(0x50, x, 0x00)
        logging.debug("Writing memory address %x with 0x00" % x)
        time.sleep(0.5)
        byte = bus.read_byte_data(0x50, x)
...
def WriteSensorType():
    ...
    bus.write_byte_data(0x50, 0x15, 0x03)
    time.sleep(0.5)
    byte_h = bus.read_byte_data(0x50, 0x15)
    bus.write_byte_data(0x50, 0x16, 0x01)
    time.sleep(0.5)
    byte_h = bus.read_byte_data(0x50, 0x15)
    byte_l = bus.read_byte_data(0x50, 0x16)
...
def ClearCalibrationData():
    ...
    for x in clear_list:
        for y in range (0x00, 0x0f):
            bus.write_byte_data(0x50, x+y, 0x00)
            logging.debug("Writing memory address %x" % (x+y))
            time.sleep(0.5)
            byte = bus.read_byte_data(0x50, x+y)
...
def ReadEEPROMManufacturer():
    ...
    byte = bus.read_byte_data(0x50, 0xFA)
...
def ReadEEPROMDeviceID():
    ...
    byte = bus.read_byte_data(0x50, 0xFB)
...
def WriteExampleData():
    ...
    for addr in range(0x36, 0x3A):
        ...
        bus.write_byte_data(0x50, addr, value)
